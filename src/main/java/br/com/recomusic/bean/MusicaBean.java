...
public class MusicaBean extends UtilidadesTelas implements Serializable
{
		...
		try
		{
			if((valorIdMusica!=null && valorIdMusica.length()>0) && (nomeMusica!=null && nomeMusica.length()>0) && (valorIdMusicaEcho!=null && valorIdMusicaEcho.length()>0) )
			{
				...
				if(UtilidadesTelas.verificarSessao())
				{
					...
					am = avaliarMusicaDAO.pesquisaUsuarioAvaliouMusica(valorIdMusica, getUsuarioGlobal());
	...
	public void avaliarMusica(String nota)
	{
		try
		{
			...
			if(!this.curtiuMusica)
			{
				...
				if(m!=null && m.getPkMusica()>0)
				{
					...
					{
						//Caso exista, atualiza as informações do cadastro de banda caso existam ou cria nova caso seja necessário
						imcb = null;
						imcb = informacaoMusicalCadastroBandaDAO.pesquisarIMCB(getUsuarioGlobal(), banda);
						
						if(imcb!=null && imcb.getPkInformacaoMusicalCadastroBanda()>0)
						{
						...
						for (String nomeGenero : listasGenerosBanda)
						{
							...
							genero = generoDAO.pesquisarGenero(nomeGenero);
							 ...
							 else
							 {
								 ...
								 mUG.setMediaAvaliacoes(((mUG.getMediaAvaliacoes() * (mUG.getQuantidadeMusicas()-1)) + Double.valueOf(nota))/mUG.getQuantidadeMusicas());
							 }
						...
						//Fim do if quando vai curtir uma música que não existem mas a banda já existe
					}
					else
					{
						//Como a banda não existia então cria a banda, cria os gêneros que ainda não existem, as informações de cadastro e os BandaGeneros
						 banda = new Banda();
						 banda = m.getBanda();
				...
				else
				{
					...
					m = pesquisaMusica(valorIdMusicaEcho);
					...
					else
					{
					 ...
					 //Cria a musica setando a banda, as informações de cadastro de música e a AvaliarMusica
					 m.setBanda(banda);
		...
		/*			
					if(banda!=null && banda.getPkBanda()>0)
					{
						//atualiza as informações do cadastro de banda e gênero que já devem existir
						imcb = null;
						imcb = informacaoMusicalCadastroBandaDAO.pesquisarIMCB(getUsuarioGlobal(), banda);
						...
						for (String nomeGenero : listasGenerosBanda)
						{
							...
							if(imcg!=null && imcg.getPkInformacaoMusicalCadastroGenero()>0)
							{
						...
						if(banda!=null && banda.getPkBanda()>0)
						{
							...
							for (String nomeGenero : listasGenerosBanda)
							{
								...
								genero = generoDAO.pesquisarGenero(nomeGenero);
								 ...
								 {
									 mUG.setMediaAvaliacoes(((mUG.getMediaAvaliacoes() * (mUG.getQuantidadeMusicas())) - am.getNota() + Double.valueOf(nota) )/mUG.getQuantidadeMusicas());
								 }
